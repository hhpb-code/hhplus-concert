package com.example.hhplus.concert.domain.payment;

public class PaymentConstants {

  public static final String PAYMENT_NOT_FOUND_MESSAGE = "결제 정보를 찾을 수 없습니다.";
  public static final String RESERVATION_ID_NULL_MESSAGE = "예약 ID는 필수입니다.";
  public static final String USER_ID_NULL_MESSAGE = "사용자 ID는 필수입니다.";
  public static final String AMOUNT_MUST_NOT_BE_NULL_MESSAGE = "금액은 필수입니다.";
  public static final String AMOUNT_MUST_BE_POSITIVE_MESSAGE = "금액은 0보다 커야 합니다.";
  public static final String PAYMENT_ID_NULL_MESSAGE = "결제 ID는 필수입니다.";

}