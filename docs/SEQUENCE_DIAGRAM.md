...
sequenceDiagram
    ...
    대기열 토큰 ->> 사용자: 사용자 조회 요청
    사용자 -->> 대기열 토큰: 사용자 반환
    대기열 토큰 ->> 대기열 토큰: 기존 대기열 토큰 조회
    opt 기존 대기열 토큰이 있을 경우
        대기열 토큰 ->> API: 기존 대기열 토큰 반환
        API -->> User: 기존 대기열 토큰 반환
    end
...
sequenceDiagram
    ...
    participant 대기열 토큰
    participant 사용자
    title 유저 대기열 토큰 생성 시퀀스 다이어그램
    User ->> API: 대기열 토큰 생성 API 요청
    API ->> 대기열 토큰: 대기열 토큰 생성 요청
    대기열 토큰 ->> 사용자: 사용자 조회 요청
    사용자 -->> 대기열 토큰: 사용자 반환
    opt 사용자가 없을 경우
        대기열 토큰 -->> API: 사용자 없음 반환
        API -->> User: 사용자 없음 반환
    end
...
sequenceDiagram
    ...
    loop 5초마다 재 요청 (대기열 검증 통과할때 까지)
        ...
        대기열 검증 ->> 사용자: 사용자 조회 요청
        사용자 -->> 대기열 검증: 사용자 반환
        opt 사용자가 없을 경우
            대기열 검증 ->> User: 사용자 없음 반환
        end
